...
export default function AddPageForm() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.post('http://localhost:5000/api/sitemap/add-page', { url, pageType });
...
export default function AddPageForm() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.post('https://seo-toolkit-08ge.onrender.com/api/sitemap/add-page', { url, pageType });
      alert('Page added successfully');
      setUrl('');
    } catch (err) {
      alert(err.response?.data?.message || 'Error adding page');
