...
// Controller for generating SEO-friendly keywords
...
async function updateMetaTagController(req, res) {
  ...
  // Get meta tag by pageUrl (for rendering webpages)
async function getMetaTagByPageController(req, res) {
    const { url } = req.query;

    if (!url) {
      return res.status(400).json({ message: 'Missing page URL query param' });
    }

    try {
      const metaTag = await MetaTag.findOne({ pageUrl: url });
      if (!metaTag) {
        return res.status(404).json({ message: 'No meta tag found for this URL' });
      }
